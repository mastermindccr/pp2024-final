...
// 使用SIMD同時處理16個MD5運算
void md5String_SIMD16(char *inputs[16], uint8_t *results[16]) {
    // 創建16個上下文
    MD5Context ctx[16];
...
// 修改原始的md5String函數來使用SIMD版本
void md5String(char *input, uint8_t *result) {
    // 如果只有單個輸入，我們仍然可以使用SIMD
    // 通過複製相同的輸入到4個槽位來利用SIMD
    char *inputs[4] = {input, input, input, input};
    uint8_t *results[4] = {result, NULL, NULL, NULL};
    
    md5String_SIMD(inputs, results);
}
//-------------